...
class DepartmentLogic():
    ...
    ''' 分页查询部门信息 '''
    def query_page(self, name = '', status = 0, page = 1 , size = 12):        
        sql = self._query_sql
        ...
        yz = tuple(ps)
        deps = mysql.find_page(sql, yz, self._query_col, page, size)
        if None != deps['data']:
            for r in deps['data']:
    ...
    def query_one(self, id = 0):
        ...
        if 0 != id:
            ...
            return None
        yz = tuple(ps)
        dep = mysql.find_one(sql, yz, self._query_col)
        if None != dep:
    ...
    def query_all_by_active(self):
        ...
        deps = mysql.find_all(sql, yz, self._query_all_by_active_col)
        return deps
