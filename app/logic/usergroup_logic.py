...
def query_one( id = 0):
    ...
    if 0 != id:
        ...
        return None
    yz = tuple(ps)
    usergroup = mysql.find_one(sql, yz, _query_col)
    if None != usergroup:
...
def query_page_user_groups( userID, page = 1, size = 12):
    isdelete = state.Boole['false']
    yz = (userID, isdelete, state.statusActive)
    users = mysql.find_page(_query_user_groups_sql, yz, _query_user_groups_col, page, size)
    return users
...
def query_page_group_roles( userGroupID, page = 1, size = 12):
    isdelete = state.Boole['false']
    yz = (userGroupID, isdelete, state.statusActive)
    roles = mysql.find_page(_query_group_roles_sql, yz, _query_group_roles_col, page, size)
    return roles
...
''' 删除用户组的角色 '''
def del_group_role( id, userGroupID, user):
    isdelete = state.Boole['true']
    yz = (isdelete, user, id)
    result = mysql.insert_or_update_or_delete(_del_group_role_sql, yz)
    return 0 == result
...
''' 清除用户组下所有用户缓存 '''
def _del_user_group_cache( userGroupID):
    key = _users_by_group_key % str(userGroupID)
